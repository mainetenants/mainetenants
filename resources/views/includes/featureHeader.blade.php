...
<section>
    <div class="feature-photo">
        ...
        <div class="add-btn">
            <span class="text-white">{{ $get_total_followers }} followers</span>
            ...
            @if($frnd_status == 1)
                ...
                @if($follow_status1 == '1')
                    <span>
                        <a href="{{ url('unfollow/'.$id ) }}" title=""
                            class="btn btn-primary unfollow-btn" data-ripple="">unfollow</a>
                    </span>
                @elseif($follow_status1 == '0')
                    <span>
                        <a href="{{ url('follow/'.$id ) }}" title=""
                            class="btn btn-primary unfollow-btn" data-ripple="">follow</a>
                    </span>
                @else

                @endif
            @elseif($frnd_status == 0)
                <a href="{{ url('cancel_request/'.$id ) }}" title=""
                    data-ripple="">Requested</a>
            @else
                <a href="{{ url('add_friend/'.$id ) }}" title="" data-ripple="">Add Friends</a>
            @endif

        </div>
        <form action="{{ url('cover_photo') }}" class="edit-phto" id="cover_form"
                ...
                <div class="col-lg-2 col-sm-3">
                    <div class="user-avatar">
                        <figure>
                            ...
                            @else
                                <img src="{{ asset('assets/images/resources/user-avatar.jpg') }}"
                                    alt="">
                            @endif
                            <form action="{{ url('profile_photo') }}" class="edit-phto"
                                id="profile_form" enctype="multipart/form-data" method="POST">
                                <i class="fa fa-camera-retro"></i>
                                <label class="fileContainer">
                                    Edit Display Photo
